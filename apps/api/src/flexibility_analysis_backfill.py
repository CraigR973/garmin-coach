"""One-off backfill for historical post-flexibility analyses."""
...
async def run_backfill(*, since: datetime | None, commit: bool) -> None:
    async with AsyncSessionLocal() as session:
        ...
        for profile in profiles:
            if commit:
                results = await service.generate_for_pending_flexibility(
                    profile,
                    since=since,
                    commit=False,
                )
                ...
                pending_count += len(
                    await service.pending_flexibility_activities(profile.id, since=since)
                )
    ...
    print(
        f"post-flexibility backfill {mode}: profiles={len(profiles)} "
        f"pending={pending_count} generated={generated} existing={existing}"
    )


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Backfill post-flexibility analyses.")
    parser.add_argument(
        "--since",

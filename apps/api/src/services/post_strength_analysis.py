...
class PostStrengthAnalysisService(PostActivityReadRunner[StrengthAnalysisResult]):
    ...
    async def pending_strength_activities(
        ...
        *,
        ...
        strength_rows = [activity for activity in rows if is_strength_activity(activity)]
        activity_ids = [activity.id for activity in strength_rows]
    ...
    # one of them a type name, a service name or the literal "strength".
    kind = "strength"
    analysis_type = ANALYSIS_TYPE
    prompt_version = PROMPT_VERSION

    async def assemble_packet(self, player: Profile, activity: Activity) -> dict[str, Any]:
        return await self.assemble_strength_packet(player, activity)

    def build_user_prompt(self, context_packet: dict[str, Any]) -> str:
        return build_strength_user_prompt(context_packet)

    def default_client(self) -> PostActivityClient:
        return AnthropicStrengthAnalysisClient()

    def make_result(self, analysis: Analysis, *, generated: bool) -> StrengthAnalysisResult:
        return StrengthAnalysisResult(analysis=analysis, generated=generated)

...
**Batch 37 — Collapse-not-remove Home sections — IMPLEMENTED on `feat/batch-37-collapse-home-sections`, pushed for review (NOT shipped).** Frontend-only Home rearchitecture (DECISIONS #109; spec `docs/designs/home-collapse-not-remove.md`). Supersedes the Batch 24 remove-model (the "read-first Home" of #91/#97): instead of `DashboardPage` rendering one of three phase blocks with off-phase sections *absent*, Home now renders the **full six-section superset every load** — `lastNight` / `today` / `afterRide` / `tomorrow` / `tonight` / `bedroom` — with exactly **one expanded** per data state and the rest **collapsed-but-present** (one-line summary in the header), so nothing disappears as the day turns. New reusable `components/CollapsibleSection.tsx` (header summary + chevron; **body rendered only while open**, so a collapsed data-heavy section — e.g. the sleep card's `useBedroomOvernight` query — costs nothing). New pure `lib/homeSections.ts`: `PRIMARY_BY_PHASE` (pre-ride→`today`, post-ride→`afterRide`, rest-day→`lastNight`), `orderedSections(phase, {hasRide, isEvening})`, `isEveningNow()`. `DashboardPage` builds a `sections` record and renders `orderedSections(...).map(...)`; the old phase cards became body components (`SleepSnapshotBody`/`DayPlanBody`/`PostRideBody`/`TomorrowBody`/`SleepPrepBody`/`BedroomBody`), day label + verdict badge on the Today header (badge shown once — #107 preserved). **Presence is data-gated, never phase-gated:** only `afterRide`/`tomorrow` can be absent, and only when no ride was analysed today — `lastNight` + `today` always stay on Home post-ride (the object-permanence win). **Lazy-body consequence:** the collapsed Last-night summary is payload-only (duration + qualifier + REM); the `OvernightGlance`/room-verdict sits in the lazy sleep body, so pre-ride the overnight query fires only when Mark opens the card. **Clock nudges order only:** after 20:00 local, `tonight`+`bedroom` float behind the primary — the clock never adds/hides/re-picks a section. Collapse state derived per load (no sticky manual state). **No `/api/v1/daily-loop` payload change, no backend change, no migration.** **Decision-number correction:** the ledger/spec were pre-written "#108", but the 2026-07-02 Batch 38/39-withdrawal replan took #108 and renumbered Batch 37 → **#109** — DECISIONS #109 added, ledger row + spec header reconciled. **Verification (local):** new `lib/homeSections.test.ts` (8 pure tests) + a rewritten `DashboardPage.test.tsx` around the section list (per-state expansion + collapsed-but-present summaries asserted in the DOM, expanding mounts the lazy body incl. the overnight query, evening nudge reorders via a faked `Date` without changing presence, offline renders, all Batch 29/36 workout-row interactions preserved) — full web vitest **71 passed / 18 files**, lint 0 errors, build (`tsc` + Vite) clean; backend untouched. **Next step:** branch pushed; open the PR, watch CI green, then `/closeout` (squash-merge + prod smoke) — **not** run here (`/batch-start` stops before promotion). After Batch 37 the queue is non-cycling analysis **Batch 40 → 41 → 42** plus the new **Batch 43** (post-strength), all backend+frontend (Batches 38/39 withdrawn — DECISIONS #108).
...
## Log
- **2026-07-02** — **Built Batch 37 — Collapse-not-remove Home sections — on `feat/batch-37-collapse-home-sections`.** Replaced `DashboardPage`'s three near-duplicate phase branches (Batch 24 remove-model, where off-phase sections were *absent*) with one ordered render of the six-section superset (`lastNight`/`today`/`afterRide`/`tomorrow`/`tonight`/`bedroom`): exactly one section expanded per data state, the rest collapsed-but-present with a one-line header summary — so nothing disappears from Home as the day turns. New `components/CollapsibleSection.tsx` (summary + chevron header; body rendered only while open, so a collapsed data-heavy section like the sleep card's `useBedroomOvernight` query costs nothing) and pure `lib/homeSections.ts` (`PRIMARY_BY_PHASE`, `orderedSections(phase,{hasRide,isEvening})`, `isEveningNow`). The former phase cards became body components; day label + verdict badge moved to the Today section header (badge once). Presence is data-gated (only `afterRide`/`tomorrow` absent, and only when no ride analysed — `lastNight`+`today` always present post-ride). Clock nudges ordering only (after 20:00 `tonight`+`bedroom` float behind the primary). No `/api/v1/daily-loop`/backend/migration change. **Decision-number correction:** the pre-written "#108" was consumed by the 2026-07-02 Batch 38/39 replan (DECISIONS #108) which renumbered Batch 37 → **#109** — recorded DECISIONS #109, reconciled the ledger row + spec header. Added `lib/homeSections.test.ts` (8 pure tests) + rewrote `DashboardPage.test.tsx` around the section list (per-state expansion, collapsed summaries in the DOM, expand-mounts-lazy-body incl. the overnight query, evening reorder via faked `Date`, offline, all Batch 29/36 workout-row interactions preserved). Verified: full web vitest 71 passed/18 files, lint 0 errors, build (`tsc` + Vite) clean; backend untouched. `docs/phase-batches.md` Batch 37 row left unstruck (not shipped). Ready for review; not closed out.

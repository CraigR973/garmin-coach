...
**Batch 65 — Separate cycling & strength + correct Mon/Sat mapping — IMPLEMENTED on `feat/batch-65-separate-cycling-strength`; local gates green; NOT yet shipped (awaiting review + `/closeout`).** 🔴 High, Decision **#138** (spec `docs/designs/workout-scheduling-feedback.md`), first of the two batches from Mark's 2026-07-07 feedback (66 — swap-first recovery guidance — is still `Planned`). **Root cause it fixes:** every build-week Saturday in `apps/api/data/plans/plan_no2.json` was one `bike_endurance` row "Z2 + Neuromuscular + Dumbbells" (summary literally ended "Then: 20min dumbbells"), so the day read/moved as one Cycle unit; and the Mon/Sat strength was mirror-imaged. **Key pre-work finding:** multi-workout days already work end-to-end — the schedule service returns a list per day and `WeekAheadPage` renders one Move-able `WorkoutRow` per `workout.id` via version-as-slot — so only the importer and swap-target detection assumed one workout/day. **What it does:** **(65.1)** the plan JSON is rewritten so every Monday (13 wks) is the **Dumbbell** full-body circuit (`strength_maintenance`, ~22 min, from `Dumbbell & Bodyweight 19.06.26.docx`) and each **build-week** Saturday (9 wks) splits into two same-`dow` entries — a `bike_endurance` "Z2 + Neuromuscular" (welded dumbbell tail dropped) then a **Bodyweight** `strength_maintenance` session; recovery/consolidation/taper Saturdays (wks 3/9/12/13) keep their single ride, and a semantic old-vs-new diff confirmed every other cell byte-identical (no bike prescription touched), so rows go 78 → 87. **(65.2)** `import_plan` assigns **incrementing versions per date** (cycle v1, strength v2, stable order) so the two Saturday rows satisfy `uq_planned_workouts_user_date_version`; re-import lands on the same versions. **(65.3)** `_active_workout_on` gains an optional `category` filter and `swap_day` passes the source's day-category (`category_for_workout_type`), so a moved ride swaps only with the target day's ride and never drags a same-day strength/flexibility row (swapping two ride days leaves both days' strength put); the Batch 60 completed-session 409 guards are unchanged. This intentionally changes the old cross-category behaviour (moving a ride onto a flexibility-only day used to drag the flexibility back to the vacated day; now it's a move — the ride joins that day, the flexibility stays), updated in `test_plan_actions.py`. **No migration** (version-as-slot reused); verdict / #133 / #134 / #135 / Red-never-VO2 invariants untouched. **Verification (local, green):** backend pytest **504 passed / 165 skipped** (the new importer-versioning + two category-scoped multi-workout swap tests are DB-backed → skip without local Postgres, run in CI), ruff check + `ruff format --check` clean, mypy **98 files** clean; shared typecheck + **12** tests; web vitest **180 passed / 30 files** (incl. the new split-day render guard), `tsc`/lint (0 errors, 6 pre-existing Fast-Refresh warnings)/build clean under Node 20. **Next step:** review the branch, then `/closeout` — which must run the **65.4 prod re-import forward from next Monday** (`python -m src.plan_import apps/api/data/plans/plan_no2.json --start-date <next-Monday>` so Mark's already-hand-juggled current week is preserved) then `reconcile_deliveries` to re-sync Zwift for the new bike rows, plus the prod HTTP smoke on the merge SHA. **Open (carried):** (a) the Vercel `/api/*` → Railway rewrite was reported 404ing the whole API surface after Batch 64 (infra, flagged for Craig — pre-existing, not this batch); (b) 🔒 RLS disabled on 16 `coach.*` tables; (c) genuine POOR-readiness days remain a coaching/data question; (d) runtime coaching model still `claude-sonnet-4-6`.

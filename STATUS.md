...
**Batch 33 (2026-07-01): Bedroom overnight temperature verdict — implementation ready on `feat/batch-33-bedroom-verdict`.** The existing overnight bedroom read now classifies the night into the app's same Green/Amber/Red traffic-light system without touching the fan loop: `services/bedroom_overnight.py` adds `warning_minutes` (readings `>= 19.5 °C`), `critical_minutes` (`>= 20.0 °C`), and a pure `room_verdict(...)` classifier gated by `RED_CRITICAL_MINUTES=60`; `GET /api/v1/bedroom/overnight` and `bedroomOvernightEnvelopeSchema` now carry `warningMinutes`, `criticalMinutes`, and `roomVerdict`; Home's overnight glance and the `/bedroom` chart header both render a verdict badge using the shared verdict colour mapping with room-appropriate labels (`Green` / `Amber` / `Red`). **No migration, no new endpoint, no new cloud call, no fan-threshold change.** **Verification:** backend pytest on `test_bedroom_overnight.py` + `test_bedroom.py` (`17 passed / 3 skipped` on this machine because the DB-backed bedroom endpoint tests still skip without local Postgres), backend mypy on the touched modules green, backend ruff on the touched files green; frontend targeted vitest green (`30 passed` across `bedroomChart`, `BedroomOvernightChart`, `DailyDetailPages`, `DashboardPage`), `pnpm --dir apps/web lint` clean apart from the repo's existing Fast Refresh warnings in shared UI files, and `pnpm --dir apps/web build` green. **Durable docs updated:** DECISIONS #104 records the verdict classification decision; `ARCHITECTURE.md` now notes that the overnight summary also carries the warning/critical-minute verdict. **Next step:** review this branch, then run `/closeout 33` when you want the PR/CI/merge/deploy ceremony.

...
**Batch 58 — Sleep-stage age-comparison table — IMPLEMENTED on `feat/batch-58-sleep-stage-age-comparison`; not shipped.** 🟢 Mid, additive backend/shared/frontend work with no migration and no new endpoint (DECISIONS #131; spec `docs/designs/coaching-calibration-and-data-truth.md`). Extended the existing morning-analysis `ageComparison` packet instead of adding a parallel route: `services/age_norms.py` now keeps the existing compact `rows` for Home's headline table and adds a sibling `sleepRows` group for the deeper Sleep-page read. The new sleep rows compare **Duration, Deep, Light, REM, Awake, and Restless** against coarse age-band norms, using **sleep-stage percentages** (not raw minutes) for Deep/Light/REM/Awake so the comparison tracks stage mix rather than just time in bed. `morning_analysis._age_comparison` now threads sleep duration/stages/restless count from the synced `Sleep` row, shared Zod schemas parse the widened payload, and `/sleep` renders a new dedicated "Sleep stages vs your age" table while Home's compact `MetricComparisonTable` stays unchanged. **Verification:** backend `test_age_norms.py` **9 passed**, touched backend ruff clean, touched backend mypy clean; shared schema vitest **10 passed**; focused web vitest **14 passed** across `MetricComparisonTable`, new `SleepStageAgeTable`, and `SleepPage`; web lint 0 errors (the same 6 pre-existing Fast-Refresh warnings); web build clean under Node 20. **Next step:** review the branch, then run explicit closeout only when requested.

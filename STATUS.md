...
**Batch 57 — Data truth in reviews/trends — SHIPPED (PR #79, squash `4e0497a`), prod-verified.** 🔴 High, backend/analysis-engine + small Reviews UI/schema surface, no migration and no new endpoint (DECISIONS #130; spec `docs/designs/coaching-calibration-and-data-truth.md`). This follows Batch 56's prod diagnosis: coverage was mostly present (daily metrics/sleep/activities 2025-06-24 → 2026-07-05, strength classified, active plan exists), so no speculative prod backfill was run; the existing read-only diagnosis SQL plus resumable `garmin_history_backfill.py` remain the operational path if a future coverage gap is found. **What changed:** review packets are now `PACKET_VERSION=2` / `reviews-v3-2026-07-05` and carry explicit `coverage` (expected days, sleep/recovery/verdict/thermal coverage, activity count, plan rows, adherence capture); sleep/recovery trends carry first-half → second-half means, deltas, and sample counts; adherence exposes `sourceState`/`zeroInterpretation` when no active plan rows exist; strength exposes source state when no tracked strength activity exists; Reviews API/shared schemas and `/reviews` show the coverage/trend-basis/source-state caveats. Trends prompt versions move to `trends-*-v3-2026-07-05` and require from→to numbers/sample counts for trend claims while preserving deterministic insufficient-history behaviour. **Verification:** local focused backend reviews/trends pytest **23 passed / 7 skipped**, focused Reviews/Trends web vitest **4 passed**, shared schema vitest **10 passed**, touched backend ruff/format/mypy clean, web typecheck/lint/build clean; PR #79 CI green across backend pytest/ruff/mypy, Alembic migration check, security audit, web build, web/shared vitest, and the Vercel preview. **Production verified on merge SHA `4e0497a`:** Railway and Vercel same-origin `/api/v1/health` both returned `sha=4e0497ab73de97b0b3a7eea36c1dc5a19dd755bd`, web `/` and `/reviews` returned 200, and unauthenticated `GET /api/v1/reviews/weekly` plus `GET /api/v1/trends/narrative?bucket=month` returned 401 both direct and via Vercel. **Next step:** Batch 58 — Sleep-stage age-comparison table.

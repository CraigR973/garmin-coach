...
**Pre-ship thermal + morning-flow fixes — SHIPPED (PR #77, squash `e0ee083`), prod-verified.** Not a numbered batch — a `fix/pre-ship-thermal-and-morning-flow` hardening branch bundling three coherent fixes settled with Craig on 2026-07-05 (DECISIONS #126–#128); no migration, no new endpoint/payload field. **(1) A late morning check-in now shapes today's session (backend, #126).** The wake verdict is computed from sleep/recovery *before* Mark checks in, so a check-in that lands after it (the intended Sleep → check-in → ride order puts it there) never reached the eased ride even though the framework reads the subjective score. New `ExecutableCoachingService.regenerate_after_morning_checkin` (called from `PUT /manual-entry` for the profile-local *today*) re-runs the verdict + eased ride when a check-in **worsens** it — but only while the ride is still `proposed` (never touches an approved/pushed ride — #29), only when the deterministic packet status actually drops to Amber/Red (an ordinary check-in makes no LLM call), and best-effort (a save never breaks). Subjective is downgrade-only, so this can only ever *ease* the session. **(2) Home's morning action ladder re-orders to sleep → check-in → eased ride (frontend, #127).** In the `pre_training`/`rest_day` morning phase `nextAction` now leads with "Review last night's sleep" (`/sleep`) > "Morning check-in" > the eased ride, matching Mark's real morning (read recovery before setting the ride); outside the morning it keeps the prior need-first order. A new per-day `lib/sleepReview.ts` localStorage flag (keyed by `subjectDate`, mirroring the #99 client-only Ignore) is set when he opens `/sleep`, so the sleep rung completes and steps down rather than nagging; it only fires once overnight metrics have synced (`morningAnalysis != null`). **(3) Thermal: reject a future-dated Hive device clock (backend, #128).** A boiler module was seen reporting a `lastSeen` decades ahead (a "2068" reading) that then read as perpetually fresh and shadowed the real latest temperature — `_hive_captured_at` now clamps a far-future device timestamp back to poll time (regardless of the stale-preference flag) and `is_hive_temperature_fresh` uses an absolute delta so a future reading can't pass as fresh. **Verification (local, all green):** backend ruff + `ruff format --check` + mypy clean, full pytest **475 passed / 149 skipped** (the new `regenerate_after_morning_checkin` + thermal-clamp DB cases run in CI — no local Postgres on this Mac; the pure thermal-freshness cases pass locally); shared typecheck + 10 tests; full web vitest **156 passed / 27 files** (new morning-ladder cases in `homeActions.test.ts` + `DashboardPage.test.tsx`), web typecheck clean, web lint 0 errors (6 known Fast-Refresh warnings), web build clean under Node 20. **Shipped:** PR #77 CI went green across backend pytest/ruff/mypy, Alembic migration check, security audit, web build, web/shared vitest, and the Vercel preview; squash-merged to `main` as `e0ee083` and the branch was deleted. **Production verified on merge SHA `e0ee083`:** Railway direct and Vercel same-origin (`garmin-coach-one.vercel.app`) `/api/v1/health` both returned `sha=e0ee083f73c0411358d1c24dab1548f101708bea`, web `/` and `/login` both returned 200, and `GET /api/v1/daily-loop` returned 401 unauthenticated both direct and via the Vercel rewrite. **Next step:** **Mark's 2026-07-05 feedback is captured as a tracked punch-list + fix spec in `docs/designs/coaching-calibration-and-data-truth.md`** — nine points across over-caution/calibration (the sleep gate can't be overridden by good HRV/RHR; "76 readiness eroding"; midweek recovery-day conflict), data truth (monthly/last-year gaps; "strength stopped"/"zero planned" narrated from empty inputs), the missing post-workout feed-forward loop, and additive features (sleep-stage age-comparison table; chronic-pattern suggestions). **Start with Step 0 (on-screen diagnosis with Craig, checked vs Mark)** before any build — several wrong *outputs* may be truthful reports of wrong *inputs* (plan not loaded? history depth? strength classification?). The verdict-calibration items (A1/C1) change safety behaviour → need decision #129 sign-off. This is the queued priority unless Craig picks otherwise from `docs/phase-batches.md`.

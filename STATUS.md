...
**Batch 33 (2026-07-01): Bedroom overnight temperature verdict — SHIPPED (PR #49, squash `8389985`), prod-verified.** The existing overnight bedroom read now classifies the night into the app's same Green/Amber/Red traffic-light system without touching the fan loop: `services/bedroom_overnight.py` adds `warning_minutes` (readings `>= 19.5 °C`), `critical_minutes` (`>= 20.0 °C`), and a pure `room_verdict(...)` classifier gated by `RED_CRITICAL_MINUTES=60`; `GET /api/v1/bedroom/overnight` and `bedroomOvernightEnvelopeSchema` now carry `warningMinutes`, `criticalMinutes`, and `roomVerdict`; Home's overnight glance and the `/bedroom` chart header both render a verdict badge using the shared verdict colour mapping with room-appropriate labels (`Green` / `Amber` / `Red`). **No migration, no new endpoint, no new cloud call, no fan-threshold change.** **Verification:** branch CI green across ruff, mypy, pytest, Alembic migration check, security audit, web build, and Vercel preview; production on merge SHA `8389985` verified via Railway `/api/v1/health`, Vercel same-origin `/api/v1/health`, web `/` `200`, and unauthenticated `GET /api/v1/bedroom/overnight` returning `401` both direct and through the Vercel rewrite (non-mutating auth-gated smoke). **Durable docs updated:** DECISIONS #104 records the verdict classification decision; `ARCHITECTURE.md` now captures the shipped overnight verdict surface; `docs/phase-batches.md` strikes Batch 33 `Shipped`; and the Batch 33 design doc now says shipped. **Next step:** Batch 34 — bedroom temperature × sleep correlation — is now the next unshipped batch.

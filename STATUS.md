...
**Batch 57 — Data truth in reviews/trends — IMPLEMENTED on `feat/batch-57-data-truth`, not closed out.** 🔴 High, backend/analysis-engine + small Reviews UI/schema surface, no migration and no new endpoint (DECISIONS #130; spec `docs/designs/coaching-calibration-and-data-truth.md`). This follows Batch 56's prod diagnosis: coverage was mostly present (daily metrics/sleep/activities 2025-06-24 → 2026-07-05, strength classified, active plan exists), so no speculative prod backfill was run in this implementation; the existing read-only diagnosis SQL plus resumable `garmin_history_backfill.py` remain the operational path if a future coverage gap is found. **What changed:** review packets are now `PACKET_VERSION=2` / `reviews-v3-2026-07-05` and carry explicit `coverage` (expected days, sleep/recovery/verdict/thermal coverage, activity count, plan rows, adherence capture); sleep/recovery trends carry first-half → second-half means, deltas, and sample counts; adherence exposes `sourceState`/`zeroInterpretation` when no active plan rows exist; strength exposes source state when no tracked strength activity exists; Reviews API/shared schemas and `/reviews` show the coverage/trend-basis/source-state caveats. Trends prompt versions move to `trends-*-v3-2026-07-05` and require from→to numbers/sample counts for trend claims while preserving deterministic insufficient-history behaviour. **Verification:** backend ruff + format-check clean on touched API files; touched-service/router mypy clean; focused backend reviews/trends pytest **23 passed / 7 skipped**; focused Reviews/Trends web vitest **4 passed**; web `tsc --noEmit` clean; web lint 0 errors / 6 known Fast-Refresh warnings; web build clean; shared schema vitest **10 passed** and shared typecheck clean. **Next step:** commit/push the branch, then wait for explicit `/closeout 57` before PR/CI/merge/prod smoke.

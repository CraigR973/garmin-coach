...
**Batch 56 — Verdict calibration & personal baselines — SHIPPED (PR #78, squash `20437e8`), prod-verified.** 🔴 High, backend/analysis-engine focused, no migration and no endpoint change (DECISIONS #129; spec `docs/designs/coaching-calibration-and-data-truth.md`). Step 0 prod diagnosis was run first and showed the problem is mostly calibration/schedule truth, not missing data: Garmin daily metrics/sleep/activities span 2025-06-24 → 2026-07-05, strength is classified, post-workout analyses exist, and the active plan exists, while recent Amber/Red verdicts were sleep-driven despite balanced HRV. **What changed:** `morning_analysis` prompt/version moves to `morning-analysis-v2-2026-07-05`; a soft-sleep score (60-73) may stay Green only when HRV is clean, RHR is inside Mark's personal baseline band, readiness is not low/poor, and subjective is absent or >=5; the Red floor and Red-never-VO2 remain untouched. Morning packets now carry `personalBaselines`, structured `trainingSchedule`, and a compact `yesterdayLoad` summary with linked post-session analysis snippets. Reviews/trends now carry personal baseline bands, and review readiness trends require both >5% movement and >4 absolute points before calling a trend, so 80→76 is stable noise rather than "eroding." `training_schedule` is a seeded KB section (Mon/Fri rest days), and existing users receive missing default sections through `ensure_seeded`; DB-history baselines now include `readiness_score`. **Verification:** PR #78 CI went green across backend pytest/ruff/mypy, Alembic migration check, security audit, web build, web/shared vitest, and the Vercel preview; production Railway and Vercel same-origin `/api/v1/health` both returned `sha=20437e82119aac1ad6e0c5bc7d7f6e46c01a099b`, web `/` and `/login` both returned 200, and `GET /api/v1/daily-loop` returned 401 unauthenticated both direct and via the Vercel rewrite. Local full backend pytest was **480 passed / 149 skipped** (3 existing warnings), backend ruff clean, `ruff format --check` clean, touched-service mypy clean; full `mypy apps/api/src` still hits the known local missing-stub issue in `services/dreo_fan.py` for `pydreo` (outside this batch). **Next step:** Batch 57 — Data truth in reviews/trends.

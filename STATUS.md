...
**Batch 40 — Post-flexibility/mobility analysis — IMPLEMENTED on `feat/batch-40-post-flexibility-analysis`, ready for closeout (NOT shipped).** Rebased onto current `origin/main` after Batch 37 shipped, so the branch now contains only the intended Batch 40 work plus its docs. Backend + frontend, no migration (DECISIONS #110; spec `docs/designs/post-flexibility-analysis.md`). New `services/post_flexibility_analysis.py` mirrors the Batch 8 post-workout machinery but with a lean mobility packet: pure name-based `is_flexibility_activity` (`"mobility"` in activity name, never `typeKey=="other"` so the 153 old misclassified rides cannot leak in; yoga excluded), pure `compute_flexibility_consistency` (streak / this-week / 4-week frequency), `assemble_flexibility_packet` (duration, avg/max HR vs resting HR, consistency, planned mobility session, activity-linked check-in, data-quality guardrails), mobility-coach prompt, Anthropic Messages boundary, `pending_flexibility_activities`, and idempotent `generate_for_pending_flexibility` storing `analyses.analysis_type='post_flexibility'` with `activity_id`. `/api/v1/daily-loop` now serializes a dedicated `postFlexibilityAnalyses` list; shared Zod schemas parse it; Home renders an advisory "Flexibility read" markdown card inside the Today section on mobility days. Scheduler `run_garmin_activity_poll` runs the flexibility pass after ride analysis and logs mobility counts separately. New one-off `python -m src.flexibility_analysis_backfill --since 2026-06-01 [--commit]` helper dry-runs by counting pending sessions and only calls the model with `--commit`, for the ~47 historical mobility sessions. **Open calls settled:** analyse all mobility sessions including the 3-minute routine; keep sibling service now; dedicated daily-loop list; backfill full routine history. **Recovery isolation preserved:** advisory only, never feeds Green/Amber/Red or ride recovery, and the packet deliberately omits power/FTP/zones/cadence/stamina/Performance Condition/Training Effect/time-series. **Verification before rebase:** backend targeted pytest for `test_post_flexibility_analysis.py` + `test_daily_loop.py` **7 passed / 8 skipped**; full backend pytest **389 passed / 134 skipped**; backend ruff check + format-check clean; touched-source mypy clean; full backend `mypy src` still hits the pre-existing local `pydreo` missing-stub/import issue in `services/dreo_fan.py` (left untouched); shared test **7 passed** + typecheck clean; web full vitest **72 passed / 18 files**, lint 0 errors (5 existing Fast Refresh warnings), build (`tsc` + Vite/PWA) clean. **Next step:** PR/CI/merge/prod smoke for `/closeout 40`; Batch 40 row remains unstruck until closeout completes.

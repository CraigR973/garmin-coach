...
**Batch 68 - Grade what he actually rode - IMPLEMENTED on `feat/batch-68-delivered-ir-grading` (verified locally, NOT yet shipped).** 🔴 High, Decision **#141**, spec `docs/designs/workout-fidelity-and-feedback-loop.md`. The post-workout packet now grades against the latest **pushed delivery proposal IR** for the planned ride (prefer `planned_workout_id`, fallback to same-date legacy delivery rows), so an accepted Amber/Red/manual override is treated as the workout Mark actually rode; only when no delivered proposal exists does it fall back to the planned row. The packet also carries a `gradingTarget` block (`source`, proposal id/status, event id, `origin`, `adjustment`) plus the planned-workout adherence entry (`workoutAdherence`, including `actualWorkoutJson.changeSummary`), and the prompt now tells Claude to name the accepted substitution/override rather than narrating a stale VO2 attempt. `PROMPT_VERSION` bumped to `post-workout-analysis-v4-2026-07-09`, so stale ride reads regenerate through the corrected target. **Boundaries:** no migration; no grading-math rewrite; whole-ride-average context guardrails unchanged; verdict / #133 / #135 / Red-never-VO2 untouched. **Gates (local):** focused post-workout pytest **12 passed / 8 skipped**; full backend pytest **528 passed / 169 skipped**; backend ruff format/check clean; touched-file mypy clean. Full backend mypy still hits the known unrelated local `pydreo` missing-stub issue in `services/dreo_fan.py`. Web/shared not touched. **Next step:** open PR; then Batch **69** remains next (`accept=logged; move session-logging post-ride`). **Open (carried):** (a) `garmin-coach.vercel.app` stale alias `/api/*` 404 — use `garmin-coach-one.vercel.app` for same-origin smoke; (b) RLS disabled on 16 `coach.*` tables; (c) genuine POOR-readiness days remain a coaching/data question; (d) runtime coaching model still `claude-sonnet-4-6`.

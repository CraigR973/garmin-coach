...
Batch 30 (DECISIONS #100; implementation-ready branch, not shipped yet) adds a
thin plan-action boundary over the real `planned_workouts` rows:
...
- [ ] **Batch 30 — Home day controls + rearrangeable week plan implementation ready** on `feat/batch-30-home-day-controls` (DECISIONS #100): adds the `plan-actions` schedule/action boundary over real `planned_workouts`, day labels for cycle/weights/flexibility/rest/mixed, Home day-level add/skip/actual controls while preserving per-workout Today-card actions, and a live rearrangeable Plan page with add/move/skip/rest-day swap-in controls. Not shipped until explicit closeout merges, verifies CI/prod, and updates docs on `main`.

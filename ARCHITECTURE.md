...
- [x] **Batch 28 — age-comparison axis + merged last-night table shipped** (PR #42, squash `0738c2a`, DECISIONS #98): shows Mark, on Home, how last night sits against both his own normal **and** the typical person his age — one glanceable card, no new Garmin call, no migration. `services/age_norms.py` encodes static sex×decade population norms for VO₂max / RHR / overnight HRV (privacy-safe, ~50th-pct guides) and `build_age_comparison` classifies each metric **direction-aware** (`good`/`neutral`/`warn`; a RHR *below* average reads green). Garmin fitness age is read from the already-synced `daily_metrics.raw_payload['max_metrics_vo2'][0].generic.fitnessAge` (computed server-side, no longer surfaced directly since the merged-table design made a standalone banner redundant). The age axis is merged with the existing `metricsVsBaselines` read into a single `MetricComparisonTable` (4-col: Metric / Last night / vs your normal / vs your age) embedded inside the 'Last night's sleep' card, replacing the REM/Deep/SpO₂ stat grid; comparison columns are difference-forward (verdict + numeric delta, `in range` / `N above/below`; `—` where a frame is absent). `AgeComparisonCard` deleted. Frontend-only after the backend additions; `/baselines` detail route keeps `MetricsBaselineTable`. Green/Amber/Red verdict untouched.

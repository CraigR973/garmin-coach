...
- **Auth:** name + PIN (bcrypt) + JWT access/refresh, **plus an additive passwordless device-token path now live alongside** (auth-simplification Phase 1 — DECISIONS #73–74/#77; PIN removal is the later Phase 3). Stripped of WC's leagues/groups/invites/email — **1–2 private users, no sign-up**.
...
- [x] **Auth simplification Phase 1** — additive passwordless device-token activation live alongside PIN (migration `008`; dual-path `get_current_user`; `POST /api/v1/auth/activate`; `python -m src.activate` CLI; `/activate` PWA route — DECISIONS #73-74/#77). Phases 2-3 (hide PIN, then delete PIN/JWT/lockout — closes P1-1/P1-3/P3-1/2/3) pending

...
- [ ] **Batch 41 — Walking integration implemented on branch** (`feat/batch-41-walking-integration`, DECISIONS #111): adds a deterministic walking brief (`services/walking_brief.py`, `GET /api/v1/walking-brief`, `walkingBrief` on `/api/v1/daily-loop`), advisory active-recovery walk volume in the morning packet (`classificationImpact: none`), and a threshold-gated deliberate-walk post-session path (`analysis_type='post_walk'`) using a lean HR/pace packet with HR-zone distribution and no power/FTP keys. The hourly Garmin poll runs the walk pass after ride + flexibility analysis, Home shows a "Walking base" panel plus "Walk read" cards, and `walk_analysis_backfill.py --since ... [--commit]` handles historical qualifying walks. No migration; not shipped until closeout.

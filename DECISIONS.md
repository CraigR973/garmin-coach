...
19. **Batch tags are TOOL+MODEL aware, not Claude-only.** WC's `phase-batches.md` tagged each batch with a Claude model (🟢 Sonnet / 🔴 Opus). Here each batch is tagged with the intended **agent** — Claude (Opus/Sonnet) **or** Codex (+model) — because either tool may drive the work. `batch-start` states which tool/model the batch expects; `next-batch-prompt` emits a paste-ready kickoff for that tagged tool. *Default heuristic (Craig to confirm/adjust, may be cost-driven):* **Claude Opus** = complex reasoning (analysis-engine prompt design, verdict/scoring logic, scheduler/sync edge cases, debugging); **Claude Sonnet / Codex** = well-specified implementation (CRUD, components, migrations, tests, mechanical refactors).
